...
const vec3 morningSky[2] = vec3[](vec3(0.6, 0.7, 0.8), vec3(0.8, 0.85, 0.9));
const vec3 noonSky[2] = vec3[](vec3(0.6, 0.8, 1.0), vec3(0.7, 0.9, 1.0));
const vec3 sunsetSky[3] = vec3[](vec3(1.0, 0.5, 0.3), vec3(0.8, 0.4, 0.6), vec3(0.4, 0.2, 0.5));
const vec3 nightSky[2] = vec3[](vec3(0.02, 0.02, 0.1), vec3(0.05, 0.05, 0.2));
...
//functions to calculate gradients with different index counts
vec3 gradient3(vec3 rayDir, const vec3 gradient[3]) {
    float t = clamp(rayDir.y, 0.0, 1.0);
    float stepSize = 1.0 / 2.0;
    if (t < stepSize) {
        return mix(gradient[0], gradient[1], t / stepSize);
    } else if (t < 2.0 * stepSize) {
        return mix(gradient[1], gradient[2], (t - stepSize) / stepSize);
    }
    return gradient[2];
}

vec3 gradient2(vec3 rayDir, const vec3 gradient[2]) {
    float t = clamp(rayDir.y, 0.0, 1.0);
...
vec3 interpolateSkyColor(float time, vec3 rayDir) {
    if (time < 0.3 && time > 0.25) {
        ...
        return gradient2(rayDir, morningSky);
    } else if (time < 0.8) {
        return gradient2(rayDir, noonSky);
    } else if (time < 0.85) {
        return gradient3(rayDir, sunsetSky);
    } else {
        return gradient2(rayDir, nightSky);
    }
}
...
void main() {
    ...
    float dayTime = mod(u_Time / 2400.0, 1.0);
